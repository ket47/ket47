    ...
    <div style="grid-template-columns: 30px 60px auto auto auto 30px;" class="item_table">
        ...
        <?php foreach ($product_list as $i=>$product): ?>
            <div style="display: contents" class="<?= $product->is_disabled ? 'item_disabled' : '' ?> <?= $product->deleted_at ? 'item_deleted' : '' ?>">
                ...
                <div data-field="product_name.<?=$product->product_id?>"><?= $product->product_name ?></div>
                <div data-field="product_quantity.<?=$product->product_id?>"><?= $product->product_quantity ?></div>
                <div data-field="product_price.<?=$product->product_id?>"><?= $product->product_price ?></div>
